...
autoLink {
  addr: rh.usecase.2.queue
  connection: remote-amq-mesh-amqps-route <1>
  dir: out
}
...
----
<1> I'm using the AMQPs connection to zoneB as I want to multicast twice,
so i want delivery (message) to transfer to ZoneB as if it coming from an external (normal) client sending messages to (rh.usecase.2.queue). See next 
